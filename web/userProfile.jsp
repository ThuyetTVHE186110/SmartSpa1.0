        ...
        <%@page contentType="text/html" pageEncoding="UTF-8" %>
            <%@ page import="model.Person" %>
                <% // No need to declare session manually; it's already available in JSP // You can directly use session
                    ...
                    is not found or account is not in session response.sendRedirect("adminLogin.jsp"); } else { // Get
                    the account object from session Account account=(Account) session.getAttribute("account"); if
                    (account.getRole()==1 || account.getRole()==2 || account.getRole()==3) { // Allow access to the page
                    (do nothing and let the JSP render) } else { // Set an error message and redirect to an error page
                    ...
                    <body>
                        <% Account account=(Account) session.getAttribute("account"); String displayName=(account
                            !=null) ? account.getPersonInfo().getName() : "Guest" ; %>

                            <% Person person=account.getPersonInfo(); // Lấy các thông tin khác từ person String
                                fullName=(person !=null) ? person.getName() : "Guest" ; String phone=(person !=null) ?
                                person.getPhone() : "N/A" ; String email=(person !=null) ? person.getEmail() : "N/A" ;
                                String address=(person !=null) ? person.getAddress() : "N/A" ; char gender=(person
                                !=null) ? person.getGender() : 'N' ; java.sql.Date dob=(person !=null) ?
                                person.getDateOfBirth() : null; String dateOfBirth=(dob !=null) ? new
                                java.text.SimpleDateFormat("yyyy-MM-dd").format(dob) : "N/A" ; // Lấy đường dẫn hình ảnh
                                từ person %>
                                                ...
                                                <div class="card">
                                                    <div class="card-body pt-3">
                                                        ...
                                                        <div class="tab-content pt-2">
                                                            ...
                                                            ? " img/" + person.getImage() : "img/default-avatar.jpg"
                                                                                %>" alt="Profile">
                                                                            <div class="pt-2">
                                                                                <!-- Image upload input -->
